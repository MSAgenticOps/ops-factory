...
class KeywordExtractorTest {
    ...
    void shouldExtractKeywordsFromEnglishText() {
        List<String> keywords =
            KeywordExtractor.extract("Machine learning algorithms process training data to build predictive models", 5);
    ...
    void shouldExtractKeywordsFromChineseText() {
        List<String> keywords = KeywordExtractor.extract("知识库管理系统提供了文档检索和智能问答功能，支持多种文件格式", 5);
    ...
    void shouldFilterOutEnglishStopWords() {
        List<String> keywords =
            KeywordExtractor.extract("the system has very good algorithms and can do many things", 20);
    ...
    void shouldFilterOutChineseStopWords() {
        List<String> keywords = KeywordExtractor.extract("这个系统可以提供检索功能，但是需要优化", 20);
    ...
    void shouldRespectMaxKeywordsLimit() {
        List<String> keywords = KeywordExtractor.extract(
            "machine learning deep learning neural network algorithm data processing training model prediction", 3);
    ...
    void shouldReturnKeywordsSortedByFrequency() {
        List<String> keywords = KeywordExtractor.extract("服务器 服务器 服务器 监控 监控 告警", 10);
