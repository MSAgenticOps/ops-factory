...
class KnowledgeSourceValidationIntegrationTest extends KnowledgeApiIntegrationTestSupport {
    ...
    void createSourceRejectsInvalidNameCharacters() throws Exception {
        JsonNode response =
            readJson(mockMvc.perform(post("/api/knowledge/sources").contentType(MediaType.APPLICATION_JSON).content("""
                {
                  "name": "bad/name",
    ...
    void updateSourceRejectsBlankNameAfterTrim() throws Exception {
        ...
        JsonNode response = readJson(
            mockMvc
                .perform(patch("/api/knowledge/sources/{sourceId}", sourceId).contentType(MediaType.APPLICATION_JSON)
                    .content("""
                        {
                          "name": "   "
                        }
    ...
    void updateSourceRejectsDuplicateName() throws Exception {
        ...
        JsonNode response = readJson(
            mockMvc
                .perform(patch("/api/knowledge/sources/{sourceId}", sourceA).contentType(MediaType.APPLICATION_JSON)
                    .content("""
                        {
                          "name": "source-b"
                        }
