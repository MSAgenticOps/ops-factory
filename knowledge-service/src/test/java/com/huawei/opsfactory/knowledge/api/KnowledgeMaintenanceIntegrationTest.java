...
class KnowledgeMaintenanceIntegrationTest extends KnowledgeApiIntegrationTestSupport {
    ...
    void indexProfileConfigChangesRequireRebuildButRetrievalConfigChangesDoNot() throws Exception {
        ...
        mockMvc.perform(put("/api/knowledge/sources/{sourceId}/config/retrieval-profile", sourceId)
        ...
        mockMvc.perform(put("/api/knowledge/sources/{sourceId}/config/index-profile", sourceId)
            .contentType(MediaType.APPLICATION_JSON)
            .content("""
                {
                  "config": {
    ...
    void maintenanceModeBlocksSearchAndIngest() throws Exception {
        ...
        jdbcTemplate.update(
            ...
            "知识库重建中，请稍后再试", sourceId);

        var searchResult =
            readJson(mockMvc.perform(post("/api/knowledge/search").contentType(MediaType.APPLICATION_JSON).content("""
                {
                  "query": "runbook",
                  "sourceIds": ["%s"]
                }
                """.formatted(sourceId))).andExpect(status().isConflict()).andReturn());
        assertThat(searchResult.path("code").asText()).isEqualTo("SOURCE_IN_MAINTENANCE");

        MockMultipartFile file = new MockMultipartFile("files", "maintenance-check.txt", MediaType.TEXT_PLAIN_VALUE,
            "runbook maintenance check".getBytes(StandardCharsets.UTF_8));
        var ingestResult = readJson(
    ...
    void rebuildSourceRunsAsAsyncJobAndClearsPendingFlagOnSuccess() throws Exception {
        ...
        JsonNode rebuildResponse = readJson(mockMvc.perform(post("/api/knowledge/sources/{sourceId}:rebuild", sourceId))
            .andExpect(status().isOk())
            .andReturn());
    ...
    void maintenanceOverviewExposesLatestJobAndFailureDetails() throws Exception {
        ...
        jdbcTemplate.update(
            ...
            "Source rebuild completed with failures", "admin", 3, 3, 2, 1, null, null, "1 个文档处理失败",
        ...
        jdbcTemplate.update(
            ...
            "2026-03-30T05:09:00Z");

        JsonNode overview = readJson(mockMvc.perform(get("/api/knowledge/sources/{sourceId}/maintenance", sourceId))
            .andExpect(status().isOk())
            .andReturn());
        ...
        JsonNode failures = readJson(
            mockMvc.perform(get("/api/knowledge/jobs/{jobId}/failures", jobId)).andExpect(status().isOk()).andReturn());
    ...
    private JsonNode getSource(String sourceId) throws Exception {
        return readJson(
            mockMvc.perform(get("/api/knowledge/sources/{sourceId}", sourceId)).andExpect(status().isOk()).andReturn());
    }

    private void uploadRunbook(String sourceId) throws Exception {
        MockMultipartFile file = new MockMultipartFile("files", "sample-runbook.txt", MediaType.TEXT_PLAIN_VALUE, """
            # Sample Runbook
    ...
    private JsonNode waitForJob(String jobId) throws Exception {
        for (int attempt = 0; attempt < 40; attempt++) {
            JsonNode job = readJson(
                mockMvc.perform(get("/api/knowledge/jobs/{jobId}", jobId)).andExpect(status().isOk()).andReturn());
