...
public class DocumentRepository {
    ...
    public void insert(DocumentRecord record) {
        jdbcTemplate.update(
            ...
            record.id(), record.sourceId(), record.name(), record.originalFilename(), record.title(),
            record.description(), Jsons.write(objectMapper, record.tags()), record.sha256(), record.contentType(),
            record.language(), record.status(), record.indexStatus(), record.fileSizeBytes(), record.chunkCount(),
            record.userEditedChunkCount(), record.errorMessage(), record.updatedBy(), record.createdAt().toString(),
    ...
    public List<DocumentRecord> findBySourceId(String sourceId) {
        return jdbcTemplate.query("select * from knowledge_document where source_id = ? order by created_at desc",
            mapper, sourceId);
    }
    ...
    public Optional<DocumentRecord> findBySourceIdAndSha256(String sourceId, String sha256) {
        return jdbcTemplate
            .query("select * from knowledge_document where source_id = ? and sha256 = ?", mapper, sourceId, sha256)
            .stream()
            .findFirst();
    }
    ...
    public void update(DocumentRecord record) {
        jdbcTemplate.update(
            ...
            record.title(), record.description(), Jsons.write(objectMapper, record.tags()), record.contentType(),
            record.language(), record.status(), record.indexStatus(), record.fileSizeBytes(), record.chunkCount(),
            record.userEditedChunkCount(), record.errorMessage(), record.updatedBy(), record.updatedAt().toString(),
            record.id());
    }
    ...
    public long countByStatus(String status) {
        return jdbcTemplate.queryForObject("select count(*) from knowledge_document where status = ?", Long.class,
            status);
    }

    private DocumentRecord map(ResultSet rs, int rowNum) throws SQLException {
        return new DocumentRecord(rs.getString("id"), rs.getString("source_id"), rs.getString("name"),
            rs.getString("original_filename"), rs.getString("title"), rs.getString("description"),
            Jsons.readStringList(objectMapper, rs.getString("tags_json")), rs.getString("sha256"),
            rs.getString("content_type"), rs.getString("language"), rs.getString("status"),
            rs.getString("index_status"), rs.getLong("file_size_bytes"), rs.getInt("chunk_count"),
            rs.getInt("user_edited_chunk_count"), rs.getString("error_message"), rs.getString("updated_by"),
            Instant.parse(rs.getString("created_at")), Instant.parse(rs.getString("updated_at")));
    }

    public record DocumentRecord(String id, String sourceId, String name, String originalFilename, String title,
