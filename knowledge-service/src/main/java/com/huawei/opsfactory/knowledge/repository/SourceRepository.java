...
public class SourceRepository {
    ...
    public void update(SourceRecord record) {
        jdbcTemplate.update("""
            ...
            where id=?
            """, record.name(), record.description(), record.status(), record.indexProfileId(),
            record.retrievalProfileId(), record.runtimeStatus(), record.runtimeMessage(), record.currentJobId(),
            record.lastJobError(), record.rebuildRequired() ? 1 : 0, record.updatedAt().toString(), record.id());
    }
    ...
    private SourceRecord map(ResultSet rs, int rowNum) throws SQLException {
        return new SourceRecord(rs.getString("id"), rs.getString("name"), rs.getString("description"),
            rs.getString("status"), rs.getString("storage_mode"), rs.getString("index_profile_id"),
            rs.getString("retrieval_profile_id"), rs.getString("runtime_status"), rs.getString("runtime_message"),
            rs.getString("current_job_id"), rs.getString("last_job_error"), rs.getInt("rebuild_required") != 0,
            Instant.parse(rs.getString("created_at")), Instant.parse(rs.getString("updated_at")));
    }

    public record SourceRecord(String id, String name, String description, String status, String storageMode,
