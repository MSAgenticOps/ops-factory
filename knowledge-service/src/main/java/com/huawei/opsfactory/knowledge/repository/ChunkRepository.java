...
public class ChunkRepository {
    ...
    public void insert(ChunkRecord record) {
        jdbcTemplate.update(
            ...
            Jsons.write(objectMapper, record.titlePath()), Jsons.write(objectMapper, record.keywords()), record.text(),
            record.markdown(), record.pageFrom(), record.pageTo(), record.tokenCount(), record.textLength(),
            record.contentHash(), record.editStatus(), record.updatedBy(), record.createdAt().toString(),
    ...
    public List<ChunkRecord> findByDocumentId(String documentId) {
        return jdbcTemplate.query("select * from document_chunk where document_id = ? order by ordinal asc", mapper,
            documentId);
    }

    public List<ChunkRecord> findBySourceId(String sourceId) {
        return jdbcTemplate.query("select * from document_chunk where source_id = ? order by updated_at desc", mapper,
            sourceId);
    }
    ...
    public void update(ChunkRecord record) {
        jdbcTemplate.update(
            ...
            record.ordinal(), record.title(), Jsons.write(objectMapper, record.titlePath()),
            Jsons.write(objectMapper, record.keywords()), record.text(), record.markdown(), record.pageFrom(),
            record.pageTo(), record.tokenCount(), record.textLength(), record.contentHash(), record.editStatus(),
            record.updatedBy(), record.updatedAt().toString(), record.id());
    }
    ...
    public long countBySourceId(String sourceId) {
        return jdbcTemplate.queryForObject("select count(*) from document_chunk where source_id = ?", Long.class,
            sourceId);
    }

    public long countUserEdited() {
        return jdbcTemplate.queryForObject("select count(*) from document_chunk where edit_status = 'USER_EDITED'",
            Long.class);
    }

    public long countUserEditedBySourceId(String sourceId) {
        return jdbcTemplate.queryForObject(
            "select count(*) from document_chunk where source_id = ? and edit_status = 'USER_EDITED'", Long.class,
            sourceId);
    }

    private ChunkRecord map(ResultSet rs, int rowNum) throws SQLException {
        return new ChunkRecord(rs.getString("id"), rs.getString("document_id"), rs.getString("source_id"),
            rs.getInt("ordinal"), rs.getString("title"),
            Jsons.readStringList(objectMapper, rs.getString("title_path_json")),
            Jsons.readStringList(objectMapper, rs.getString("keywords_json")), rs.getString("text"),
            rs.getString("markdown"), (Integer) rs.getObject("page_from"), (Integer) rs.getObject("page_to"),
            rs.getInt("token_count"), rs.getInt("text_length"), rs.getString("content_hash"),
            rs.getString("edit_status"), rs.getString("updated_by"), Instant.parse(rs.getString("created_at")),
            Instant.parse(rs.getString("updated_at")));
    }

    public record ChunkRecord(String id, String documentId, String sourceId, int ordinal, String title,
        List<String> titlePath, List<String> keywords, String text, String markdown, Integer pageFrom, Integer pageTo,
        int tokenCount, int textLength, String contentHash, String editStatus, String updatedBy, Instant createdAt,
        Instant updatedAt) {
    }
}
