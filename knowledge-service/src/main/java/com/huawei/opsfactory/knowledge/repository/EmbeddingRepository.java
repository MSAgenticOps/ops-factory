...
public class EmbeddingRepository {
    ...
    public Map<String, EmbeddingRecord> findByContentHashes(String model, int dimension,
        Collection<String> contentHashes) {
        if (contentHashes == null || contentHashes.isEmpty()) {
            return Map.of();
        }

        List<String> hashes = contentHashes.stream().filter(Objects::nonNull).distinct().toList();
        if (hashes.isEmpty()) {
            return Map.of();
        }
        ...
        List<EmbeddingRecord> records =
            jdbcTemplate.query("select * from embedding_cache where model = ? and dimension = ? and content_hash in ("
                + placeholders + ")", mapper, concatParams(model, dimension, hashes));
    ...
    public void upsert(String contentHash, String model, int dimension, List<Double> vector) {
        ...
        jdbcTemplate.update("delete from embedding_cache where content_hash = ? and model = ? and dimension <> ?",
            contentHash, model, dimension);
    ...
    private EmbeddingRecord map(ResultSet rs, int rowNum) throws SQLException {
        return new EmbeddingRecord(rs.getString("id"), rs.getString("content_hash"), rs.getString("model"),
            rs.getInt("dimension"), readVector(rs.getString("vector_json")), Instant.parse(rs.getString("created_at")),
    ...
    public record EmbeddingRecord(String id, String contentHash, String model, int dimension, List<Double> vector,
