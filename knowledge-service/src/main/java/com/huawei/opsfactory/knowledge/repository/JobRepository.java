...
public class JobRepository {
    ...
    public void insert(JobRecord record) {
        jdbcTemplate.update("""
            ...
            """, record.id(), record.jobType(), record.sourceId(), record.documentId(), record.status(),
            record.progress(), record.stage(), record.message(), record.createdBy(), record.totalDocuments(),
            record.processedDocuments(), record.successDocuments(), record.failedDocuments(),
            record.currentDocumentId(), record.currentDocumentName(), record.errorSummary(),
            stringify(record.startedAt()), stringify(record.finishedAt()), record.createdAt().toString(),
    ...
    public void update(JobRecord record) {
        jdbcTemplate.update("""
            ...
            where id=?
            """, record.status(), record.progress(), record.stage(), record.message(), record.createdBy(),
            record.totalDocuments(), record.processedDocuments(), record.successDocuments(), record.failedDocuments(),
            record.currentDocumentId(), record.currentDocumentName(), record.errorSummary(),
            stringify(record.startedAt()), stringify(record.finishedAt()), record.updatedAt().toString(), record.id());
    }
    ...
    public Optional<JobRecord> findLatestCompletedBySourceId(String sourceId) {
        return jdbcTemplate.query(
            ...
            mapper, sourceId).stream().findFirst();
    }
    ...
    private JobRecord map(ResultSet rs, int rowNum) throws SQLException {
        return new JobRecord(rs.getString("id"), rs.getString("job_type"), rs.getString("source_id"),
            rs.getString("document_id"), rs.getString("status"), rs.getInt("progress"), rs.getString("stage"),
            rs.getString("message"), rs.getString("created_by"), rs.getInt("total_documents"),
            rs.getInt("processed_documents"), rs.getInt("success_documents"), rs.getInt("failed_documents"),
            rs.getString("current_document_id"), rs.getString("current_document_name"), rs.getString("error_summary"),
            parse(rs.getString("started_at")), parse(rs.getString("finished_at")),
    ...
    public record JobRecord(String id, String jobType, String sourceId, String documentId, String status, int progress,
        String stage, String message, String createdBy, int totalDocuments, int processedDocuments,
