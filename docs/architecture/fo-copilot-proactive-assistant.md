...
- **页头面包屑下拉** `助理 ▸ [FO Copilot ▾]`:切 copilot,**常驻下拉(单个也显示、项数不限)**;头是 **48px 对齐头**(与 sidebar / 右栏同线)。侧栏入口名为「助理」,挂**聚合未读 badge**(各 copilot 合计,复用 Inbox 那套)。
...
- **消息渲染现代化(共享层,chat + 助理页同步生效)**:改 `platform/chat` 的共享消息渲染器,**不针对单页**——① 去掉 user/assistant 的圆形头像(`Message.tsx` / `MessageList.tsx` 不再渲染 `.message-avatar`;孤儿 `UserAvatarIcon` 删除,`GooseAvatarIcon` 仍被首页用,保留);② 统一为**单列、左对齐、全宽**消息(取消 user 右对齐气泡),user 用**浅色全宽底纹块**(Claude 风)与透明的 assistant 区分;③ **消息列宽 = composer 宽**:让消息滚动区与 composer 采用同一框模型(padding 在外、`max-width:880` 在内),消除原先 `.message`(800)与 composer(880)+ `.chat-messages` 双层 padding 造成的错位,`scrollbar-gutter: stable both-edges` 保证经典滚动条平台也对齐。因走共享层,resume 页(ChatPage)自动同步、无需改其逻辑。
