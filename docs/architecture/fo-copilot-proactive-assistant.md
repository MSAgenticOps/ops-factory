...
- **C 推送卡重做(向 Inbox/History 列表项看齐)**:由"2 行预览压最上"改为**信封式三层**——概览行(schedule 类型色点 + 名称,右侧 run 时间)/ 标题行(简报首行,突出)/ 摘要行(`snippetOf` 取正文次行,muted);内边距抬到 `spacing-4`、卡间距 `spacing-3`;色点按 schedule 类型(`scheduleAccent`:brief/watch/memory/default)着色,呼应状态色语言。
