...
- **FO Copilot agent 配置**(`gateway/agents/fo-copilot/`):
  ...
  - ticket MCP:`config/mcp/` 下接入(adapter 由项目侧实现 8 个工具:`get_todo` / `get_state_context` / `get_candidates` / `create` / `update` / `comment` / `transition` / `update_assignment`)。
...
- **定时任务(每用户运行态,非静态配置)**:`ticket-watch-loop`(主动看护,cron `0 */30 * * * *`,每 30 分钟轮询)、`fo-copilot-memory-maintenance`(每天 12:00 记忆维护);二者在常驻实例首启时 seed/注册进该用户的 `data/scheduled_recipes/<id>.yaml` + `data/schedule.json`(由 goosed `/schedule/create` 生成,机制类比 PR1 记忆 seed),**不签入 `config/`**。
