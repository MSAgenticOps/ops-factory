...
### 13.2 信息架构(贴合现有「主区 + 右栏」框架,不新增全局布局区域)
...
- **页头面包屑下拉** `Thread ▸ [FO Copilot ▾]`:切 thread/copilot(单个时即标题)。侧栏新增「Thread」入口,挂**聚合未读 badge**(各 copilot 合计,复用 Inbox 那套)。
- **B(主区)= 主对话**:直接 `resume(binding.sessionId)`,**复用现有 chat 组件**(含 composer,在此回复)。无主 session 时,中间给一段提示说明。
- **C(右栏)= 主动推送时间线**:常驻 rail,**默认打开、可关可再开**;按 `targetKey` 列出该 thread 下**所有**定时任务投递(**统一样式、不分任务内容**),时间倒序。
