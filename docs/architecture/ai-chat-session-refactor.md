...
| HTTP 状态 | 含义 | 前端处理 |
| --- | --- | --- |
| 200/202 | cancel 已提交或 goosed 认为幂等成功 | 进入 `cancelling`，等待 events 或 Gateway 确认后显示 cancelled |
...
- cancel 请求失败不能伪装成已取消。
- 如果 cancel HTTP 成功但没有收到后续取消确认，前端应显示“取消已提交，等待确认”一类状态，而不是立刻删除上下文。
- 取消确认来源可以是 goosed/Gateway 的 terminal 或 cancelled 事件，也可以是 events 重连后 `ActiveRequests` 已不再包含该 `request_id`。goosed 当前不提供独立 cancelled 事件，因此不能把“HTTP cancel accepted”等同于“任务已取消”。
...
- `cancelling`: 用户已请求取消。
- `cancelled`: goosed/Gateway 通过 terminal/cancelled event 确认取消，或 events 重连后确认该 `request_id` 已不在 `ActiveRequests`。
- `failed`: 收到不可恢复错误。
- `completed`: 收到 finish。
...
- `web-app/src/app/platform/chat/useChat.ts`
  ...
  - stop 流程调整为：
    ...
    - cancel accepted 后进入 `cancelling`，继续保留 listener 等待 terminal/cancelled event，或等待 `ActiveRequests` 确认该 request 消失。
