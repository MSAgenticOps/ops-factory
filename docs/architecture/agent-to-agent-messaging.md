...
- **结果回交 A 综合**：AgentB 的最终结果作为工具返回值交给 AgentA 的模型，由 AgentA 组织成给用户的最终答复（用户始终在与 AgentA 一个声音对话）。AgentB 的过程在折叠的轻量状态里可见。
...
- **触发方式（已定：确定性解析为主、提示兜底）**：
  - **确定性部分**：前端 @ 补全产出**规范化的精确 `agentId`**（来自 agents 列表，杜绝拼错/编造）；发送链路对目标做校验（存在、可达）。
  - **提示兜底**：在各 Agent 的 AGENTS.md / 系统提示中加入规则：「当用户消息包含规范化的 `@<agentId>` 时，调用 `call_agent`，`target=<agentId>`，`message=` 去除 @ 标记后的原话」。
  - **为何仍需模型在环**：本设计要求最终结果回到 **AgentA 的模型**综合（用户一个声音对话），且 goosed 的 reply 接口不支持网关强制某次工具调用（无 tool_choice）。因此「确定性」消灭的是**选错/编造目标**这类错；「模型干脆没发起调用」只能靠强提示压低、做不到 100% 杜绝。该层「做多深」（仅前端规范化 / 加网关校验 / 强提示）留实现期微调。
...
5. **触发方式**：**确定性解析（前端补全产出规范 `agentId` + 校验）+ 系统提示兜底**；模型仍需在环发起调用（综合所需，§5.3）。
