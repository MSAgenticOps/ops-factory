...
## 配置生效与实例重启
- Agent 的模型/provider 配置（`GOOSE_PROVIDER`、`GOOSE_MODEL`、API key 等标量键）在拉起 `goosed` 时以环境变量注入，而 Goose 的配置读取以环境变量优先于 config.yaml 文件。因此对已运行实例而言，这些配置在进程生命周期内是冻结的，保存配置文件不会改变其行为。
- 生效路径只有重建进程：`POST /api/gateway/agents/{id}/instances/restart` 会停止该 Agent 全部用户实例并返回停止数量；常驻实例随后异步重新拉起，非常驻实例由下一次请求懒启动。
- 模型配置、custom provider 及 API key 的写入口在响应中返回 `requiresRestart` 与 `runningInstances`，供前端在保存后提示重启；重启会中断该 Agent 所有用户进行中的对话和调度任务。
