...
`gateway/users/<userId>/channels/<type>/<channelId>/`

这样拆分后，`gateway/channels` 可以安全地表达“系统里有哪些共享渠道、默认接入哪个 agent”，而不会混入真实账号凭据、外部联系人消息、出站队列、helper 日志和 session 绑定状态。

### 2.2 channel 配置共享，运行态按用户隔离

每个 channel 配置不再保存 `ownerUserId`。`gateway/channels/<type>/<channelId>/config.json` 是共享的 OOTB/平台配置；不同平台用户可以基于同一个 `channelId` 建立自己的登录态、bindings、events、dedup 和消息队列。

运行态目录必须由当前平台用户决定，而不是由配置文件、外部联系人 ID 或 helper 进程自行决定。当当前用户为 `admin`、`type = whatsapp`、`channelId = whatsapp-main` 时：

- 配置目录：`gateway/channels/whatsapp/whatsapp-main/`
- 运行态目录：`gateway/users/admin/channels/whatsapp/whatsapp-main/`

当另一个用户 `alice@example.com` 使用同一个 channel 时：

- 共享配置目录仍然是：`gateway/channels/whatsapp/whatsapp-main/`
- 独立运行态目录是：`gateway/users/alice@example.com/channels/whatsapp/whatsapp-main/`

API response 中短期保留的 `ownerUserId` 字段表示“当前响应所合并的运行态用户”，不是配置所有者，也不会写入 `config.json`。
...
channel 运行态迁移到 `gateway/users/<userId>/channels/...` 后，消息最终进入 agent 的 session 运行态仍然使用既有目录：

`gateway/users/<userId>/agents/<agentId>/`

channel 自身运行态和 agent session 运行态同属于 `gateway/users`，但目录职责不同：

- `users/<userId>/channels/...`：外部渠道登录态、消息队列、绑定和排障状态
...
- `ChannelRuntimeStorageService`（建议新增）
  - 集中负责按当前平台用户和 `ChannelDetail` 解析运行态目录和运行态文件路径
...
1. 校验 `id`、`name`、`type`、`defaultAgentId`
2. 写入 `gateway/channels/<type>/<channelId>/config.json`
3. 为当前请求用户创建 `gateway/users/<userId>/channels/<type>/<channelId>/`
4. 初始化运行态文件：
   - `bindings.json`
   - `events.json`
   - `inbound-dedup.json`
5. 记录 `channel.created` 事件到当前用户运行态目录下的 `events.json`
...
普通配置更新不允许写入运行态归属。不同用户是否使用某个 channel，由该用户是否存在对应运行态目录和登录态决定。
...
- `gateway/channels/<type>/<channelId>/`
- 所有 `gateway/users/<userId>/channels/<type>/<channelId>/`
...
binding 会生成 `syntheticUserId`，但启动或复用 agent session 时使用当前运行态用户。目标态选择的模型是：

- 外部会话的 agent runtime 使用当前运行态用户
- `syntheticUserId` 只作为 binding 标识或 session metadata，不作为 `gateway/users/<userId>` 的真实目录层级

原因是外部渠道登录态已经按平台用户隔离，用户运行态也落在 `gateway/users/<userId>/channels/...`。如果再把外部联系人映射成 `gateway/users/channel__...`，会重新制造一套不易管理的“伪用户运行态”。

如果后续需要每个外部会话隔离 agent 工作目录，应显式设计为：

`gateway/users/<userId>/channel-sessions/<type>/<channelId>/<bindingId>/agents/<agentId>/`
...
- session 真值只保存在 `gateway/users/<userId>/agents/<agentId>/...`
...
3. 通过 `ChannelRuntimeStorageService` 定位 `gateway/users/<userId>/channels/whatsapp/<channelId>/`
4. 清理旧 PID 对应 helper
5. 创建运行态目录：
   - `auth/`
   - `inbox/`
...
3. 通过 `ChannelRuntimeStorageService` 定位 `gateway/users/<userId>/channels/wechat/<channelId>/`
...
| 运行态目录 | `users/<userId>/channels/whatsapp/<channelId>` | `users/<userId>/channels/wechat/<channelId>` |
...
1. `gateway/users/<userId>/channels/<type>/<channelId>/login-state.json`
   - 看当前状态、二维码、最近错误、最近连接时间
2. `gateway/users/<userId>/channels/<type>/<channelId>/events.json`
   - 看 gateway 侧是否已经完成 binding、reply、outbox enqueue
3. `gateway/users/<userId>/channels/<type>/<channelId>/login.log`
   - 看 helper 是否启动、登录和退出
4. `gateway/users/<userId>/channels/<type>/<channelId>/whatsapp-debug.log`
   - 看 WhatsApp helper 是否持续收到消息、是否发送失败
5. `gateway/users/<userId>/channels/<type>/<channelId>/inbox/` 和 `outbox/`
...
- message pump 扫描 `users/<userId>/channels/.../inbox`
- outbox 写入 `users/<userId>/channels/.../outbox/pending`
- binding 写入 `users/<userId>/channels/.../bindings.json`
- dedup 写入 `users/<userId>/channels/.../inbound-dedup.json`
- event 写入 `users/<userId>/channels/.../events.json`
...
- 登录服务启动 helper 时，断言传入的是 `gateway/users/<userId>/channels/...` 路径
...
- 所有新增运行态文件默认落到 `gateway/users/<userId>/channels/<type>/<channelId>/`
