...
Token 运营是 OpsFactory 面向智能体使用事实的运营分析模块。它基于 `gateway/users` 下 goosed session SQLite 数据库，围绕 `user`、`agent`、`session`、`model`、`provider` 和 Token 维度回答：
...
- `snapshot-dir` 已在配置模型中保留，但当前实现没有把快照持久化到磁盘。
...
`SessionDbReader` 以只读方式打开 SQLite：
...
读取失败的 DB 会被跳过，并记录到 `SnapshotStatus.skippedDbCount` 和 `lastRefreshError`。服务不修改 goosed session DB，不迁移 schema，不写入 session DB。
...
└── store
    ...
    └── SessionDbReader.java
```

职责：

- `SessionDbReader`：扫描并只读解析 goosed session DB。
...
- `data-root` 是实际数据源。
- `refresh-interval-ms` 和 `max-db-open-ms` 已使用。
- `retention-days` 当前仅在配置模型中存在，尚未驱动持久化清理。
- `snapshot-dir` 当前未落盘使用。
...
`GET /finops/sessions/{sessionId}/messages` 额外要求：
...
- 快照为内存态，服务重启后需要重新扫描。
- `snapshot-dir`、`retention-days` 当前是预留配置，不代表已落盘和清理。
