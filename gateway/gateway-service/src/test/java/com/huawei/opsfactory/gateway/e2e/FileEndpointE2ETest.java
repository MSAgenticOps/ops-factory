 ...
 * GET  /agents/{agentId}/files/list
 * GET  /agents/{agentId}/files/get?path=...
 * PUT  /agents/{agentId}/files/update?path=...
 * DELETE /agents/{agentId}/files/delete?path=...
...
public class FileEndpointE2ETest extends BaseE2ETest {
    ...
    public void listFiles_authenticated_returnsFileList() throws IOException {
        ...
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/list")
    ...
    public void listFiles_unauthenticated_returns401() {
        webClient.get().uri("/api/gateway/agents/test-agent/files/list").exchange().expectStatus().isUnauthorized();
    }
    ...
    public void getFile_existingTextFile_returnsInlineContent() {
        ...
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/get?path=data/readme.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void getFile_binaryFile_returnsAsAttachment() {
        ...
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/get?path=archive.zip")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void getFile_notFound_returns404() {
        ...
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/get?path=nonexistent.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void getFile_unauthenticated_returns401() {
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/get?path=data/secret.txt")
            .exchange()
            .expectStatus()
            .isUnauthorized();
    }
    ...
    public void getFile_nestedPath_resolvesCorrectly() {
        ...
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/get?path=data/subdir/nested.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void getFile_rootId_resolvesFromScanRoot() {
        ...
        webClient.get()
            .uri("/api/gateway/agents/test-agent/files/get?path=report.md&rootId=output")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void updateFile_existingTextFile_returnsUpdated() throws IOException {
        ...
        webClient.put()
            .uri("/api/gateway/agents/test-agent/files/update?path=data/readme.md")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
    ...
    public void updateFile_notFound_returns404() throws IOException {
        ...
        webClient.put()
            .uri("/api/gateway/agents/test-agent/files/update?path=data/missing.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(Map.of("content", "Updated"))
    ...
    public void updateFile_unsupportedType_returns415() {
        ...
        webClient.put()
            .uri("/api/gateway/agents/test-agent/files/update?path=deck.pptx")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(Map.of("content", "Updated"))
    ...
    public void updateFile_pathTraversal_returns403() {
        webClient.put()
            .uri("/api/gateway/agents/test-agent/files/update?path=..%2F..%2Fsecret.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(Map.of("content", "Updated"))
    ...
    public void updateFile_unauthenticated_returns401() {
        webClient.put()
            .uri("/api/gateway/agents/test-agent/files/update?path=data/readme.txt")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(Map.of("content", "Updated"))
            .exchange()
            .expectStatus()
    ...
    public void deleteFile_existingFile_returnsDeleted() throws IOException {
        ...
        webClient.delete()
            .uri("/api/gateway/agents/test-agent/files/delete?path=data/readme.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void deleteFile_notFound_returns404() throws IOException {
        ...
        webClient.delete()
            .uri("/api/gateway/agents/test-agent/files/delete?path=data/missing.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void deleteFile_pathTraversal_returns403() {
        webClient.delete()
            .uri("/api/gateway/agents/test-agent/files/delete?path=..%2F..%2Fsecret.txt")
            .header(HEADER_SECRET_KEY, SECRET_KEY)
            .header(HEADER_USER_ID, "alice")
            .exchange()
            .expectStatus()
    ...
    public void uploadFile_unauthenticated_returns401() {
        webClient.post()
            .uri("/api/gateway/agents/test-agent/files/upload?sessionId=s1")
            .exchange()
            .expectStatus()
            .isUnauthorized();
    }
