  ...
  try {
    ...
    if (!res.ok) {
      ...
      const error = new Error(
        `Control Center ${path} returned ${res.status}: ${text}`,
      );
    ...
    if (
      !(
        ...
        error.message.startsWith(`Control Center ${path} returned`)
      )
    ) {
    ...
    inputSchema: {
      ...
      properties: {
        serviceId: {
          type: 'string',
          description:
            'Managed service id such as gateway, knowledge-service, or business-intelligence.',
        },
      },
      required: ['serviceId'],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lines: {
          type: 'number',
          description:
            'Number of trailing log lines to read (default: 200, max: 1000).',
          minimum: 1,
          maximum: 1000,
        },
      },
...
export async function handleGetServiceStatus(serviceId) {
  const status = await cc(
    `/control-center/services/${encodeURIComponent(serviceId)}`,
  );
...
export async function handleReadServiceLogs(serviceId, lines) {
  const logs = await cc(
    `/control-center/services/${encodeURIComponent(serviceId)}/logs`,
    { lines },
  );
...
export async function handleReadServiceConfig(serviceId) {
  const config = await cc(
    `/control-center/services/${encodeURIComponent(serviceId)}/config`,
  );
...
export async function handleStartService(serviceId) {
  const result = await cc(
    `/control-center/services/${encodeURIComponent(serviceId)}/actions/start`,
    undefined,
    { method: 'POST' },
  );
...
export async function handleStopService(serviceId) {
  const result = await cc(
    `/control-center/services/${encodeURIComponent(serviceId)}/actions/stop`,
    undefined,
    { method: 'POST' },
  );
  return JSON.stringify(result, null, 2);
...
export async function handleRestartService(serviceId) {
  const result = await cc(
    `/control-center/services/${encodeURIComponent(serviceId)}/actions/restart`,
    undefined,
    { method: 'POST' },
  );
  return JSON.stringify(result, null, 2);
  ...
  try {
    ...
    switch (name) {
      ...
      case 'read_service_logs':
        result = await handleReadServiceLogs(
          requireServiceId(args),
          normalizeLines(args.lines),
        );
        break;
