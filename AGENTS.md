...
Treat [`docs/architecture/overview.md`](./docs/architecture/overview.md), [`docs/architecture/api-boundaries.md`](./docs/architecture/api-boundaries.md), [`docs/architecture/process-management.md`](./docs/architecture/process-management.md), and [`docs/development/ui-guidelines.md`](./docs/development/ui-guidelines.md) as the source of truth for cross-team work. Do not bypass the gateway from the frontend, do not change auth headers or SSE/event payloads without explicit review, and keep new UI work aligned with the existing route/layout/right-panel model. Any new config key must be added to the matching `config.yaml.example` and documented in the relevant development or architecture doc.
