...
`ops-factory` is a multi-service monorepo. Core services live in [`gateway/`](./gateway), [`knowledge-service/`](./knowledge-service), [`business-intelligence/`](./business-intelligence), and [`web-app/`](./web-app). The gateway is a Java 21 Maven project split into `gateway-common` and `gateway-service`; application code is under `src/main/java` and tests under `src/test/java`. The knowledge service is an independent Spring Boot service. The current `business-intelligence/` directory contains report-generation assets and reference data that may be promoted into an independent service. The React/Vite frontend keeps code in `web-app/src`, with pages in `src/pages`, shared UI in `src/components`, hooks in `src/hooks`, and tests in `src/__tests__`. Cross-service tests live in [`test/`](./test), the TypeScript SDK in [`typescript-sdk/`](./typescript-sdk), and Docker-backed helpers in [`langfuse/`](./langfuse), [`onlyoffice/`](./onlyoffice), and [`prometheus-exporter/`](./prometheus-exporter). Agent-specific configs and skills are under `gateway/agents/*`.
...
- All charts and data visualizations in the frontend should use ECharts. Do not introduce hand-built SVG, CSS-bar, canvas, or ad hoc chart renderers unless the work is not actually a chart or there is an explicit reviewed exception.
...
Treat [`docs/architecture/overview.md`](./docs/architecture/overview.md), [`docs/architecture/api-boundaries.md`](./docs/architecture/api-boundaries.md), [`docs/architecture/process-management.md`](./docs/architecture/process-management.md), and [`docs/development/ui-guidelines.md`](./docs/development/ui-guidelines.md) as the source of truth for cross-team work. The frontend may call `gateway`, `knowledge-service`, and `business-intelligence` directly via explicit service base URLs in runtime config; do not add direct browser calls to providers or local agent runtimes. Do not change auth headers or SSE/event payloads without explicit review, and keep new UI work aligned with the existing route/layout/right-panel model and shared visual primitives. Any new config key must be added to the matching config example file and documented in the relevant development or architecture doc.
